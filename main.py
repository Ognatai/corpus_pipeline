...
from utils import save_knowledgebase, visualise_aggregated_report
from analysis import process_single_text, compile_aggregated_report
import spacy
...
def save_human_readable_report(aggregated_report, results_dir, year, temp_dir):
    ...
    try:
        ...
        with open(report_path, "w", encoding="utf-8") as file:
            ...
            file.write(f"Mostly Gender Neutral: {aggregated_report['contains_majority_gender_neutral']}\n")
            file.write(f"Only Generic Masculine: {aggregated_report['generic_masculine']}\n")

            # Add word usage
            file.write("Feminine Coded Words:\n")
            file.write(f"  Total: {aggregated_report['total_feminine_coded_words']}\n")
            file.write(f"  By Gender: {aggregated_report['feminine_coded_words_gender_distribution']}\n\n")

            file.write("Masculine Coded Words:\n")
            file.write(f"  Total: {aggregated_report['total_masculine_coded_words']}\n")
            file.write(f"  By Gender: {aggregated_report['masculine_coded_words_gender_distribution']}\n\n")

            # Add averages and medians
            ...
            # Add top PMI words
            file.write("\nTop PMI Words:\n")
            for word, score in aggregated_report['top_pmi_words']:
                file.write(f"  {word}: {score:.2f}\n")
    ...
    try:
        ...
        for article_id, text in raw_texts.items():
            try:
                ...
                knowledgebase = process_single_text(doc)
